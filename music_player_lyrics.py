...
class MusicPlayer(QMainWindow):
    def __init__(self):
        ...
        # 检测系统是否为深色主题
        self.is_dark_theme = self.palette().color(QPalette.Window).lightness() < 128
        ...
        ### 歌词显示部分
        self.current_lyrics = []  # 存储解析后的歌词：[(时间毫秒, 歌词文本), ...]
        ...
        # 设置歌词颜色
        self.setup_lyric_colors()  # 设置歌词颜色
        ...
        # 歌词调整信息
        self.lyrics_adjust = {}  # 歌词调整信息
        self.current_song_id = ""  # 当前歌曲的唯一标识
        self.load_lyrics_adjust()  # 加载歌词调整信息
    ...
    def setup_ui(self):
        ...
        lyrics_layout.setSpacing(5)  # 设置行间距

        # 上一句歌词
        self.prev_lyric_label = QLabel("")
        self.prev_lyric_label.setAlignment(Qt.AlignCenter)
        self.prev_lyric_label.setFont(QFont("Microsoft YaHei", 11))
        self.prev_lyric_label.setStyleSheet("color: #666666;")  # 灰色
        self.prev_lyric_label.setTextInteractionFlags(Qt.NoTextInteraction)  # 禁止选择

        # 当前歌词（字体稍大）
        self.current_lyric_label = QLabel("")
        self.current_lyric_label.setAlignment(Qt.AlignCenter)
        self.current_lyric_label.setFont(QFont("Microsoft YaHei", 14, QFont.Bold))  # 字体稍大并加粗
        self.current_lyric_label.setStyleSheet("color: #000000;")  # 黑色
        self.current_lyric_label.setTextInteractionFlags(Qt.NoTextInteraction)  # 禁止选择

        # 下一句歌词
        self.next_lyric_label = QLabel("")
        self.next_lyric_label.setAlignment(Qt.AlignCenter)
        self.next_lyric_label.setFont(QFont("Microsoft YaHei", 11))
        self.next_lyric_label.setStyleSheet("color: #666666;")  # 灰色
        self.next_lyric_label.setTextInteractionFlags(Qt.NoTextInteraction)  # 禁止选择
        ...
        # 歌词调整和显示部分
        lyrics_container = QHBoxLayout()

        # 添加向前调整按钮
        self.lyric_back_btn = QPushButton("◀ 歌词太快")
        self.lyric_back_btn.setFixedWidth(90)
        self.lyric_back_btn.clicked.connect(lambda: self.adjust_lyrics_timing(-500))  # 向前调整0.5秒
        self.lyric_back_btn.setToolTip("后退0.5秒")
        lyrics_container.addWidget(self.lyric_back_btn)
        ...
        # 添加到歌词布局
        lyrics_layout.addWidget(self.prev_lyric_label)
        lyrics_layout.addWidget(self.current_lyric_label)
        lyrics_layout.addWidget(self.next_lyric_label)

        # 将歌词布局添加到容器中
        lyrics_container.addLayout(lyrics_layout, stretch=1)  # 设置拉伸因子让歌词区域占据主要空间

        # 添加向后调整按钮
        self.lyric_forward_btn = QPushButton("歌词太慢 ▶")
        self.lyric_forward_btn.setFixedWidth(90)
        self.lyric_forward_btn.clicked.connect(lambda: self.adjust_lyrics_timing(500))  # 向后调整0.5秒
        self.lyric_forward_btn.setToolTip("延后0.5秒")
        lyrics_container.addWidget(self.lyric_forward_btn)

        # 添加歌词容器到主布局
        main_layout.addLayout(lyrics_container)
    ...
    # 设置歌词颜色
    def setup_lyric_colors(self):
        """根据主题设置歌词颜色"""
        if self.is_dark_theme:
            # 深色主题
            self.current_lyric_color = "#FFFFFF"  # 白色
            self.other_lyric_color = "#AAAAAA"    # 浅灰色
        else:
            # 浅色主题
            self.current_lyric_color = "#000000"  # 黑色
            self.other_lyric_color = "#666666"    # 深灰色
        
        # 应用颜色
        self.current_lyric_label.setStyleSheet(f"color: {self.current_lyric_color};")
        self.prev_lyric_label.setStyleSheet(f"color: {self.other_lyric_color};")
        self.next_lyric_label.setStyleSheet(f"color: {self.other_lyric_color};")

    # 加载歌词时间调整信息
    def load_lyrics_adjust(self):
        """加载歌词时间调整信息"""
        try:
            script_dir = os.path.dirname(os.path.abspath(__file__))
            lyrics_dir = os.path.join(script_dir, "music_download", "lyrics")
            
            # 确保目录存在
            if not os.path.exists(lyrics_dir):
                os.makedirs(lyrics_dir)
            
            adjust_file = os.path.join(lyrics_dir, "lyrics_adjust.json")
            
            if os.path.exists(adjust_file):
                with open(adjust_file, 'r', encoding='utf-8') as f:
                    self.lyrics_adjust = json.load(f)
            else:
                self.lyrics_adjust = {}
                # 创建空文件
            ...
            print(f"加载歌词调整信息时出错: {str(e)}")
            self.lyrics_adjust = {}

    # 保存歌词时间调整信息
    def save_lyrics_adjust(self):
        """保存歌词时间调整信息"""
        try:
            script_dir = os.path.dirname(os.path.abspath(__file__))
            lyrics_dir = os.path.join(script_dir, "music_download", "lyrics")
            ...
            with open(adjust_file, 'w', encoding='utf-8') as f:
                json.dump(self.lyrics_adjust, f, ensure_ascii=False, indent=2)
        except Exception as e:
            print(f"保存歌词调整信息时出错: {str(e)}")

    # 获取歌曲的唯一标识
    def get_song_id(self, song_path):
        """获取歌曲的唯一标识"""
        if not song_path:
            return ""
    ...
    # 调整歌词时间偏移
    def adjust_lyrics_timing(self, offset_ms):
        """调整歌词时间偏移"""
        if not self.current_song_id or not self.current_lyrics:
            return
        
        # 获取当前偏移
        current_offset = self.lyrics_adjust.get(self.current_song_id, 0)
        
        # 计算新偏移
        new_offset = current_offset + offset_ms
        
        # 更新偏移值
        self.lyrics_adjust[self.current_song_id] = new_offset
        
        # 保存调整信息
        self.save_lyrics_adjust()
        
        # 根据新的偏移值重新计算并显示歌词
        current_position = self.player.position()
        self.update_lyrics_display(current_position)
        
        # 显示调整信息
        self.statusbar.showMessage(f"歌词时间已调整: {new_offset/1000:.1f}秒", 2000)
    ...
    def position_changed(self, position):
        ...
        # 根据当前播放位置更新歌词显示
        self.update_lyrics_display(slider_position)
    ...
    ### 歌词显示部分
    # 载入歌词（分为标签在句首和句尾两种情况，分别代表开始或者结束时间）
    def load_lyrics(self, song_path):
        ...
        # 设置当前歌曲ID
        self.current_song_id = self.get_song_id(song_path)
        ...
        try:
            ...
            if not os.path.exists(lyrics_dir):
                os.makedirs(lyrics_dir)
                self.current_lyric_label.setText("当前歌曲无歌词")
                self.lyric_back_btn.setEnabled(False)
                self.lyric_forward_btn.setEnabled(False)
                return
            ...
            # 先尝试精确匹配
            exact_match = os.path.join(lyrics_dir, f"{song_filename}.lrc")
            if os.path.exists(exact_match):
                lrc_file = exact_match
            else:
                # 如果没有精确匹配，尝试部分匹配
                # 提取不含歌手信息的歌曲名（如果有的话）
                ...
                for file in os.listdir(lyrics_dir):
                    if file.endswith(".lrc") and song_name.lower() in file.lower():
                        lrc_file = os.path.join(lyrics_dir, file)
                        break
            ...
            # 检测时间标签位置（句首/句尾）
            is_time_at_beginning = True
            for line in lrc_content:
                line = line.strip()
                if line and not line.startswith('[ti:') and not line.startswith('[ar:') and not line.startswith('[al:'):
                    # 检查第一个非空非元数据行的时间标签位置
                    if re.search(r'\[\d+:\d+(?:\.\d+)?\]', line):
                        is_time_at_beginning = line.startswith('[')
                        break
            
            print(f"歌词格式检测: 时间标签位于{'句首' if is_time_at_beginning else '句尾'}")
            
            # 解析歌词内容
            parsed_lyrics = []
            
            if is_time_at_beginning:
                # 时间标签在句首的标准LRC格式
                for line in lrc_content:
                    line = line.strip()
                    ...
                    for time_tag in time_tags:
                        ...
                        if '.' in parts[1]:
                            ...
                            seconds = int(seconds)
                            # 将可能是1位或2位或3位的毫秒转换为3位毫秒值
                            milliseconds = int(milliseconds) * (10 ** (3 - len(milliseconds)))
                        else:
                            seconds = int(parts[1])
                            milliseconds = 0
                        ...
                        parsed_lyrics.append((time_ms, lyric_text))
            else:
                # 时间标签在句尾的情况
                # 从后向前处理，把时间标签作为下一句的开始时间
                for i in range(len(lrc_content)):
                    line = lrc_content[i].strip()
                    if not line or line.startswith('[ti:') or line.startswith('[ar:') or line.startswith('[al:'):
                        continue
                    ...
                    # 转换为总毫秒
                    end_time_ms = (minutes * 60 + seconds) * 1000 + milliseconds
                    
                    # 这个时间作为下一句的开始时间
                    # 如果这不是最后一行，则将这个时间作为下一句的开始
                    if i < len(lrc_content) - 1:
                        next_line = lrc_content[i+1].strip()
                        if next_line and not next_line.startswith('[ti:') and not next_line.startswith('[ar:') and not next_line.startswith('[al:'):
                            next_lyric_text = re.sub(r'\[\d+:\d+(?:\.\d+)?\]', '', next_line).strip()
                            parsed_lyrics.append((end_time_ms, next_lyric_text))
                    
                    # 对于第一句歌词，我们需要设置一个开始时间
                    if i == 0:
                        # 假设第一句从0开始
                        parsed_lyrics.append((0, lyric_text))
                
                # 如果是句尾格式，我们需要反转排序以保证正确的时间顺序
                parsed_lyrics.sort(key=lambda x: x[0])
            
            # 按时间排序歌词
            self.current_lyrics = sorted(parsed_lyrics, key=lambda x: x[0])
            
            if not self.current_lyrics:
                self.current_lyric_label.setText("歌词解析失败或歌词为空")
                self.lyric_back_btn.setEnabled(False)
                self.lyric_forward_btn.setEnabled(False)
            else:
                self.lyric_back_btn.setEnabled(True)
                self.lyric_forward_btn.setEnabled(True)
            
        except Exception as e:
            print(f"解析歌词文件时出错: {str(e)}")
            self.current_lyric_label.setText("歌词解析失败")
            self.lyric_back_btn.setEnabled(False)
            self.lyric_forward_btn.setEnabled(False)
    ...
    def update_lyrics_display(self, position):
        ...
        if self.current_song_id in self.lyrics_adjust:
            offset = self.lyrics_adjust[self.current_song_id]
            adjusted_position = position + offset
        
        # 防止adjusted_position为负值
        adjusted_position = max(0, adjusted_position)
        
        # 找到当前时间对应的歌词
        current_index = -1
        for i, (time_ms, _) in enumerate(self.current_lyrics):
            if time_ms > adjusted_position:
                break
            current_index = i
        ...
        if current_index >= 0:
            # 当前歌词
            _, current_text = self.current_lyrics[current_index]
            self.current_lyric_label.setText(current_text)
            
            # 上一句歌词
            if current_index > 0:
                _, prev_text = self.current_lyrics[current_index - 1]
                self.prev_lyric_label.setText(prev_text)
            else:
                self.prev_lyric_label.setText("")
            
            # 下一句歌词
            if current_index < len(self.current_lyrics) - 1:
                _, next_text = self.current_lyrics[current_index + 1]
                self.next_lyric_label.setText(next_text)
            else:
                self.next_lyric_label.setText("")
        else:
            # 如果没有找到匹配的歌词（例如，歌曲刚开始前的空白）
            self.prev_lyric_label.setText("")
            if self.current_lyrics:
                _, next_text = self.current_lyrics[0]
                self.current_lyric_label.setText("")
                self.next_lyric_label.setText(next_text)
            else:
                self.current_lyric_label.setText("当前歌曲无歌词")
